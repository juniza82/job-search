...
import egovframework.searchNet.common.WNCollection;
import egovframework.searchNet.common.WNDefine;
import egovframework.searchNet.common.WNSearch;
...
public class SearchNetService {
	...
	private static final Logger logger = LoggerFactory.getLogger(SearchNetService.class);
		...
		collectionNameList.stream().forEach((String collection) -> {
			...
			IntStream.range(0, thisTotalCount).forEach((int index) -> {
				...
				searchResultFieldList.stream().forEach((String field) -> {
					...
					result = result.replaceAll("class=aj>", "");
