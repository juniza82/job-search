package egovframework.searchNet.common;
...
public class WNSearch {
	...
	public List<String> getSearchResultField(String collectionName) {

		List<String> resultFieldNameList = null;
		String searchFieldString = getCollectionInfoValue(collectionName, WNDefine.RESULT_FIELD);
		String[] searchFieldArray = searchFieldString.split(",");
		if(searchFieldArray != null && searchFieldArray.length > 0) {
			resultFieldNameList = new ArrayList<>(Arrays.asList(searchFieldArray));
		} else {
			resultFieldNameList = new ArrayList<>();
		}
		return resultFieldNameList;

	}

	/**
	 * WNCollection의 wncol.COLLECTION_INFO의 설정 값을 Return한다.
	 * @param collName 검색하고자 하는 컬렉션의 이름
	 * @param target Return 대상
	 * @return 설정 값, 실패하면 Null을 반환한다.
	 */
	public String getCollectionInfoValue(String collName, int target) {
		int idx = getCollIdx(collName);
		if (idx == -1){
			appendWarnings("[WARNING] [getCollectionInfoValue] ["+collName+"] Collection name is not exist");
			return null;
		}
